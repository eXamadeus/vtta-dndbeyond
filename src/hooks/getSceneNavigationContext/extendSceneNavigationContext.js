...
const getSceneRelatedJournalEntries = (sceneId) => {
  return game.journal.filter((je) => {
    return je.data.flags && je.data.flags.vtta && je.data.flags.vtta.sceneId && je.data.flags.vtta.sceneId === sceneId
  })
}
      ...
      .then(async (json) => {
        ...
        if (result === SUBMIT_BUTTON) {
          ...
          fetch(URL, {
            ...
            .catch((error) => {
              return window.vtta.notification.show(`<h1>Sorry!</h1><p>${error}</p>`)
            })
        }
        return true
      })
