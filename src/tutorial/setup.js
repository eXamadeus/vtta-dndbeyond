export default async () => {
  ...
  // let tutorialStep = 0; //game.settings.get("vtta-dndbeyond", "tutorial-step");
  ...
  // if (tutorialStep > 0 && game.user.data._id !== game.settings.get("vtta-dndbeyond", "tutorial-user")) return;
  // let hint;
  // if (tutorialStep === 0) {
  //   let gms = game.users.entities
  //     .filter((user) => user.isGM && user.active)
  //     .map((user) => ({ _id: user.data._id, name: user.data.name }));
  //   let text = `<h1>Welcome to VTTA!</h1><p>Let's go through some easy setup steps together in order to get you started.</p>
  //   <hr />`;
  //   if (gms.length > 1) {
  //     text = `<h1>Welcome to VTTA!</h1><p>Let's go through some easy setup steps together in order to get you started.</p>
  ...
  //   hint = await window.vtta.hint.show(text, {
  //     align: "CENTER",
  //     next: true,
  ...
  //   if (
  //     game.settings.get("vtta-dndbeyond", "tutorial-user") === "" ||
  //     game.settings.get("vtta-dndbeyond", "tutorial-user") === game.user.data._id
  //   ) {
  //     game.settings.get("vtta-dndbeyond", "tutorial-user", game.user.data._id);
  //     tutorialStep++;
  ...
  //   hint = await window.vtta.hint.show(
  //     `<h1>Sidebar</h1>
  ...
  //     <p>For vtta-dndbeyond, we do need three compendiums reflecting the three categories of content we can import from D&amp;D Beyond: Monsters. Spells. Items.</p>
