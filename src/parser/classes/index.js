...
export default function parseClasses(ddb) {
  ...
  ddb.character.classes.forEach((characterClass) => {
    ...
    if (characterClass.subclassDefinition && characterClass.subclassDefinition.name) {
      ...
      item.data.description.value += '<p><strong>' + item.data.subclass + '</strong></p>'
      item.data.description.value += characterClass.subclassDefinition.description
    }
    ...
    const profs = DICTIONARY.character.skills
      .filter((skill) =>
        ddb.character.modifiers.class.filter(
          (mod) => mod.friendlySubtypeName === skill.label && classIds.includes(mod.componentId),
        ),
      )
    ...
    const castSpells =
      characterClass.definition.canCastSpells ||
